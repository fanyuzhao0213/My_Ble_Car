...
void motor_gpio_set_direction(uint8_t motor_channel, uint8_t direction)
{
    switch (motor_channel)
    {
        case 1:
			...
			{
				HAL_GPIO_WritePin(MOTOR_Direction1_GPIO_Port, MOTOR_Direction1_Pin,  GPIO_PIN_RESET);
			}else if(direction == MY_CAR_DIRECTION_BACKWARD)
			{
				HAL_GPIO_WritePin(MOTOR_Direction1_GPIO_Port, MOTOR_Direction1_Pin,  GPIO_PIN_SET);
			}else
			{
				HAL_GPIO_WritePin(MOTOR_Direction1_GPIO_Port, MOTOR_Direction1_Pin,  GPIO_PIN_RESET);
			}
        ...
        case 3:
			if(direction == MY_CAR_DIRECTION_FORWARD)
			{
				HAL_GPIO_WritePin(MOTOR_Direction3_GPIO_Port, MOTOR_Direction3_Pin,  GPIO_PIN_RESET);
			}else if(direction == MY_CAR_DIRECTION_BACKWARD)
			{
				HAL_GPIO_WritePin(MOTOR_Direction3_GPIO_Port, MOTOR_Direction3_Pin,  GPIO_PIN_SET);
			}else
			{
				HAL_GPIO_WritePin(MOTOR_Direction3_GPIO_Port, MOTOR_Direction3_Pin,  GPIO_PIN_RESET);
			}
...
void my_motor_control_task(void)
{
	...
	switch(g_ControlMotorFlag)
	{
		case CONTROL_MOTOR_DIRECTION:
			print_log("[MOTOR] set motor run and speedlevel!\r\n");
			//做了软启动速度控制 的加减速  因此不需要再停止电机
//			my_motor_car_stop();									//先让4个电机电机停止
//			HAL_Delay(1000);										//延时1S
		    // 获取当前方向
			current_car_dir = (HAL_GPIO_ReadPin(MOTOR_Direction1_GPIO_Port, MOTOR_Direction1_Pin) == GPIO_PIN_SET) ? MY_CAR_DIRECTION_BACKWARD : MY_CAR_DIRECTION_FORWARD;
			// 如果方向不一致，先停车
			if(current_car_dir != g_MotorDirection) 
			{
				print_log("[MOTOR] set motor turn direction!\r\n");
		...
		case CONTROL_MOTOR_TURN:
			print_log("[MOTOR] set motor turn!,g_MotorTurnDirection:%d!\r\n",g_MotorTurnDirection);
//			my_motor_car_stop();									//先让4个电机电机停止
//			HAL_Delay(1000);										//延时1S
//			car_turn(g_MotorTurnDirection,g_MotorTurnLevel);		//小车转向
		...
		case CONTROL_MOTOR_STOP:
			print_log("[MOTOR] set motor stop!\r\n");
